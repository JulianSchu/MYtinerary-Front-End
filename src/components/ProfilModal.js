import React, { Component } from 'react';
import { slide as Menu } from 'react-burger-menu';
import { Link } from 'react-router-dom';
import { withRouter } from 'react-router-dom';
import { connect } from  'react-redux';
import { logout } from '../actions/authActions';
import PropTypes from 'prop-types';
import '../styles/profilModal.css';
import '../styles/mytinerary.css';

export class ProfilModal extends Component {
    state = {
      menuOpen: false
    } 

    handelStateChange = (state) => {
      this.setState({menuOpen: state.isOpen})
    }

    onClick = () => {
      this.setState({
        menuOpen: false
      })
    }

    logout = () => {
      this.props.logout();
      this.setState({
        menuOpen: false
      })
    }
  
    render () {
      return (
      <Menu 
        isOpen={this.state.menuOpen} 
        onStateChange={(state) => this.handelStateChange(state)}
        right 
        noOverlay 
        customBurgerIcon={ 
          this.props.isAuthenticated ? 
          <div className="">
            <div className="profilIcon" style={{backgroundImage: `url(${this.props.user.profilPic})`}}></div>   
          </div> : 
          <i className="fas fa-user-circle"></i> }
        className="navIcon"
        >
        { this.props.user ? 
        <div className="d-flex flex-wrap">
          <div className="profilePic" style={{backgroundImage: `url(${this.props.user.profilPic})`}}></div>   
            <p className="title pt-2 mx-auto col-12 text-center">{this.props.user.userName}</p>
        </div> : null
        }
        <div className="d-flex flex-wrap justify-content-center">
        <Link className="link text-white text-center menu-item col-12" to="/" onClick={this.onClick}>Home</Link>
        <Link className="link text-white text-center menu-item col-12" to="/cities" onClick={this.onClick}>Cities</Link>
        { this.props.isAuthenticated ? 
        <p className="link text-white text-center menu-item col-12" onClick={this.logout}>Log out</p> :
        <div className="title d-flex flex-wrap justify-content-center py-3">
          <div>
            <div className="profilePic" style={{backgroundImage: 'url("data:image/png;base64,")'}}></div>   
          </div>
          <p className="col-12 text-center mt-3">Want to build your own MYtinerary?</p>
          <Link to="/LogIn" className="text-center text-white  link col-12" onClick={this.onClick}>Log in</Link>
          <p className="my-3">-- or --</p>
          <Link to="/SignUp" className="text-center text-white  link col-12" onClick={this.onClick}>Create Account</Link>
        </div>
        }
        </div>
      </Menu>
    );
  }
}

const mapStateToProps = state => ({
  // token: state.auth.token,
  isAuthenticated: state.auth.isAuthenticated,
  user: state.auth.user
});

ProfilModal.propTypes = {
// token: PropTypes.string,
  isAuthenticated: PropTypes.bool,
  user: PropTypes.object,
  logout: PropTypes.func.isRequired
}

export default connect(mapStateToProps, { logout })(withRouter(ProfilModal))
